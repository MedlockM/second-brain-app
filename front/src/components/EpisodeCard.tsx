        ...
        <div className="flex-1 p-6">
          <div className="mb-4">
            ...
            {episode.episode_date_published > 0 && (
              <div className="flex items-center text-xs text-gray-500">
                ...
                <span>Published: {formatDate(episode.episode_date_published)}</span>
              </div>
            )}
          </div>
          ...
          <div className="mb-3">
            <button
              ...
              className="flex items-center justify-between w-full px-4 py-2 text-sm font-semibold text-left bg-gradient-to-r from-blue-50 to-purple-50 hover:from-blue-100 hover:to-purple-100 rounded-lg transition-colors"
            >
              <span>Reveal quiz ({episode.quiz.questions.length} questions)</span>
              {showQuiz ? (
          ...
          <div>
            ...
            {showSummary && (
              <div className="mt-2 p-4 bg-gray-50 rounded-lg space-y-4">
                {/* Main Topics */}
                {episode.summary.main_topics && episode.summary.main_topics.length > 0 && (
                  <div>
                ...
                {/* Key Points */}
                {episode.summary.key_points && episode.summary.key_points.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-2">
                      Key points:
                    </h4>
                ...
                {/* Notable Quotes */}
                {episode.summary.notable_quotes && episode.summary.notable_quotes.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-2">
                      Notable quotes:
                    </h4>
                    <div className="space-y-2">
                      {episode.summary.notable_quotes.map((quote, index) => (
                        <blockquote
                          key={index}
