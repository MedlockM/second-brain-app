...
export default function PricingPage({
  ...
  const handleSubscribe = async (tier: string) => {
    ...
    setLoadingPlan(tier);
    try {
      const { url } = await SettingsService.createSubscriptionCheckout(
        token,
        tier,
      );
  ...
  return (
    <div className="min-h-screen mx-auto relative bg-white overflow-x-hidden">
      ...
      {/* Pricing cards */}
      <div
        className={`grid md:grid-cols-2 ${showPacks ? "lg:grid-cols-4" : "lg:grid-cols-3"} max-w-7xl gap-6 py-12 mx-auto px-4 relative z-10`}
      >
              ...
              <CardContent className="pt-0">
                ...
                >
                  {loadingPlan ===
                    (showPacks
                      ? `pack-${plan.minutes}`
                      : (plan as (typeof subscriptionPlans)[0]).tier) && (
