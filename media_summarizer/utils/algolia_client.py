...
Multi-tenant model: single shared index per environment, with tenant isolation
enforced by a ``user_id`` filter applied server-side on every query.

Index naming: ``media_items_{environment}`` (e.g. media_items_development,
media_items_production).

Security model: ALGOLIA_API_KEY (admin/write key) is used server-side for
indexing and for searching. No Algolia key ever reaches a client — the mobile
app searches through the backend.
