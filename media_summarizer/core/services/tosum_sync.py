"""
Service: Spotify Tosum Sync

Fetch listened episodes from a user playlist, resolve via PodcastIndex,
...
SPOTIFY_TOSUM_PLAYLIST_NAME = os.environ.get("SPOTIFY_TOSUM_PLAYLIST_NAME", "Tosum")
...
async def run_tosum_sync_for_user(user) -> Dict[str, Any]:
    ...
    playlist = await find_playlist_by_name(access_token, SPOTIFY_TOSUM_PLAYLIST_NAME)
    if not playlist:
        return {
            "status": "error",
            "reason": "playlist_not_found",
            "playlist_name": SPOTIFY_TOSUM_PLAYLIST_NAME,
        }
