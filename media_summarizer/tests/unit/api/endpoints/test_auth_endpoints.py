...
class TestRegisterEndpoint:
    ...
    async def test_register_success_sets_refresh_cookie_and_returns_user(self):
        ...
        with patch("media_summarizer.api.endpoints.auth.database_async.get_user_by_email", new_callable=AsyncMock) as mock_get_user_by_email, \
             patch("media_summarizer.api.endpoints.auth.database_async.create_user", new_callable=AsyncMock) as mock_create_user, \
             patch("media_summarizer.api.endpoints.auth.database_async.create_auth_token", new_callable=AsyncMock) as mock_create_token, \
             patch("media_summarizer.api.endpoints.auth.email_service.send_email_verification", new_callable=AsyncMock) as mock_send_email:
            ...
            assert mock_send_email.await_count >= 0  # background task may not run synchronously
...
class TestLoginEndpoint:
    ...
    async def test_login_success_sets_cookie_and_returns_access_token(self):
        ...
        with patch("media_summarizer.api.endpoints.auth.database_async.get_user_by_email", new_callable=AsyncMock) as mock_get_user_by_email, \
             patch("media_summarizer.api.endpoints.auth.verify_password") as mock_verify_password, \
             patch("media_summarizer.api.endpoints.auth.database_async.create_auth_token", new_callable=AsyncMock) as mock_create_token:
...
class TestRefreshEndpoint:
    ...
    async def test_refresh_success_rotates_cookie_and_returns_access_token(self):
        ...
        with patch("media_summarizer.api.endpoints.auth.database_async.get_auth_token_by_token", new_callable=AsyncMock) as mock_get_by_token, \
             patch("media_summarizer.api.endpoints.auth.database_async.update_auth_token", new_callable=AsyncMock) as mock_update_token, \
             patch("media_summarizer.api.endpoints.auth.database_async.get_user_by_id", new_callable=AsyncMock) as mock_get_user, \
             patch("media_summarizer.api.endpoints.auth.database_async.create_auth_token", new_callable=AsyncMock) as mock_create_token:
...
class TestLogoutAndMeEndpoints:
    def override_current_user(self):
        return AuthUser(id="u-1", email="u@example.com", credits=42)
    ...
    async def test_logout_revokes_tokens_and_clears_cookie(self):
        ...
        with patch("media_summarizer.api.endpoints.auth.database_async.revoke_user_tokens", new_callable=AsyncMock) as mock_revoke:
