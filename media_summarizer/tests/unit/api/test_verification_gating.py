...
def override_get_current_user():
    ...
    return AuthUser(id="test-user-1", email="u@example.com", credits=100)


def test_unverified_user_blocked_on_payments_intent(monkeypatch):
    # Override auth to return a user
    from media_summarizer.api.dependencies.auth import get_current_user
    app.dependency_overrides[get_current_user] = override_get_current_user

    # Patch DB to return an unverified local user
    from media_summarizer import utils as utils_pkg
    from media_summarizer.utils import database_async

    unverified_user = User(id="test-user-1", email="u@example.com", credits=100, auth_provider="local")
    monkeypatch.setattr(database_async, "get_user_by_id", AsyncMock(return_value=unverified_user))

    # Call payments intent (requires verified email)
    resp = client.post("/api/v1/payments/intent", json={"credits": 50, "currency": "eur"})
