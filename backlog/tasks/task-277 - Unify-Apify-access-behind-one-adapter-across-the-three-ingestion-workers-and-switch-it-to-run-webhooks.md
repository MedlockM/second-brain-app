...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A single Apify adapter module is the only place in media_summarizer/ that builds an Apify API URL or holds an Apify token, poll interval or timeout constant; the Instagram, TikTok and YouTube workers reach Apify only through it
- [ ] #2 No call site uses run-sync-get-dataset-items any more: every actor run is started through the start-run endpoint
- [ ] #3 The run ID is persisted on the processing job before the worker returns, so a run started by a worker that then dies is still identifiable from DynamoDB
- [ ] #4 No polling loop, sleep or blocking wait on an Apify run remains in any of the three workers or in the resolver; the worker returns without knowing the run's outcome
- [ ] #5 A webhook is registered per run covering the succeeded state and the terminal failure states (failed, aborted, timed-out), so a run that never succeeds still resolves its job instead of leaving it pending
- [ ] #6 The callback endpoint rejects a request that does not carry the expected shared secret, and resumes a job only when the payload's run ID matches the run ID persisted on that job
- [ ] #7 A duplicate delivery of an already-processed callback is a no-op: it does not enqueue a second Deepgram job and does not overwrite a completed job's state
- [ ] #8 Starting a run also posts a delayed backstop message with DelaySeconds; when it fires it is a no-op if the job already completed, and marks the job failed with an explicit reason otherwise
- [ ] #9 APIFY_MAX_POLLS and APIFY_POLL_INTERVAL_SECONDS are deleted rather than defaulted or disabled, and no environment variable or flag can restore the inline waiting path
- [ ] #10 The three ingestion workers' Lambda timeouts and their queues' visibility timeouts are reduced to fit the work that remains once no Apify wait happens inside them
- [ ] #11 ruff and mypy are clean, and terraform validate plus terraform plan exit 0 for the dev env
- [ ] #12 No Apify token and no webhook secret is written into any tracked file, task notes and Terraform included
<!-- AC:END -->
