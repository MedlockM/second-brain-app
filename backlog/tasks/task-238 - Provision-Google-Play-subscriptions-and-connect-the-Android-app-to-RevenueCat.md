...
- Because RevenueCat mints a public SDK key at app creation, **the real `goog_` key now exists** and can replace the `your_revenucat_google_api_key_here` placeholder in `mobile/.env` — independently of everything below. It will not make an offering resolve, but it stops Android configuring the SDK with a placeholder string.
