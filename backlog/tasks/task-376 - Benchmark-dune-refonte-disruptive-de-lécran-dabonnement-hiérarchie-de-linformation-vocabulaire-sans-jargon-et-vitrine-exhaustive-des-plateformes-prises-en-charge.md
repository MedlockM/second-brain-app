...
<!-- SECTION:NOTES:BEGIN -->
**Mode : initial.** Aucun `README.owner-rejected-*.md` ni `complement-request-*.md` dans `docs/research/task-376-*/` au lancement — premier passage sur ce benchmark.

Livrable : `docs/research/task-376-subscription-screen-redesign/README.md`, `owner_decision: pending`. Aucun fichier de `mobile/` ni de `media_summarizer/` n'a été modifié.

Contenu, section par section : état des lieux chiffré des onze locales, en mots et en caractères, replié et déplié, avec le nombre de gestes par information (§1) ; inventaire des sources ingérables dressé depuis `classifiers.py`, `SourcePlatform`, `DocumentFormat.supported_extensions()`, `UPLOAD_PICKER_MIME_TYPES` et `whatsapp-share-payload-shapes.md`, avec une section à part pour ce que l'écran ne montre pas — WhatsApp texte et note vocale en tête (§2) ; quatre partis pris de présentation, chacun avec un wireframe textuel et rattaché à des implémentations réellement consultées — Google One, Spotify Premium, Apple One, Readwise Reader, Snipd, RevenueCat (§3) ; lexique de remplacement de « transcription », « import », « minutes », « formule » et « essai » dans les onze locales, dérivé des chaînes déjà traduites du dépôt plutôt qu'inventé, avec le coût de désalignement store chiffré à 41 champs et les chemins de menus exacts d'App Store Connect et de la Play Console (§4) ; question des logos tiers tranchée avec les règles de marque Apple, TikTok, WhatsApp, Instagram et Spotify citées (§5) ; mécanisme anti-dérive fondé sur une liste servie par le backend, compatible avec « aucune figure côté mobile » (§6) ; tenue en arabe RTL, en allemand, en néerlandais, à 375 pt, et vérification ligne à ligne des invariants — prix du package store, conditions de renouvellement, liens légaux, absence de bouton « Restaurer les achats », trois états de chargement (§7) ; réponse tranchée « deux écrans, frontière déplacée » avec ses conséquences de navigation (§8). Ce qui n'a pas pu être consulté depuis l'environnement est isolé au §9.

**Reprise après interruption, sur arbre rafraîchi.** Le worktree a été remis à jour sur `main` en cours de rédaction, avec task-372 (suppression des tags) et task-373 (« collection » → « dossier ») mergées entre-temps. Toutes les mesures de l'AC#1 ont été **recomptées sur l'arbre courant** : le français passe de 869 à 863 mots pour l'ensemble `plan.*` + `paywall.*`, de 219 à 216 mots repliés et de 355 à 352 mots dépliés, les deux blocs `plan.highlight.organise` et `plan.includes.organise.file` ayant perdu la promesse des tags. Les tableaux couvrent désormais les onze locales et non plus huit. Le vocabulaire périmé a été purgé : aucune recommandation ne s'appuie sur les tags, et le mot retenu pour un dossier est celui de task-373.

**La recommandation attend la validation de l'owner.** Le parti pris recommandé est le A — suppression du dépliant, vitrine de pastilles nominatives et tableau de coût à quatre lignes à plat, « transcription » retiré de l'écran. Rien ne doit être implémenté avant que le champ `Decision` de la section Owner Validation soit renseigné ; c'est lui qui fait foi pour task-377, pas cette recommandation.

**Deux actions restent à la charge de l'owner** si le parti pris A est retenu : la mise à jour des 39 Descriptions d'abonnement dans App Store Connect (treize entrées × trois produits ; chemins de menus au §4.4 du README), et la relecture par un locuteur des formulations marquées « nouveau » au §4.2.
<!-- SECTION:NOTES:END -->
