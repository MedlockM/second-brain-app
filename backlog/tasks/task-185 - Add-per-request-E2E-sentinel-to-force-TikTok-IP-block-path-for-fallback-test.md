...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 TikTok ingestion worker detects a request-scoped sentinel in the normalised URL (e.g. query param `__e2e_force_ip_block__=1`) and raises `TikTokIPBlocked` before invoking yt-dlp
- [ ] #2 The sentinel is documented inline in the worker so it cannot be confused with a feature flag
- [ ] #3 `tests/e2e/test_fallback_chains.py::test_tiktok_apify_fallback` is updated to use a sentinel URL pointing at a TikTok video for which the Apify actor reliably returns content
- [ ] #4 The test passes deterministically against AWS dev (status=completed, `extraction_metadata.provider == "apify"` or equivalent fallback marker)
- [ ] #5 No regression on `tests/e2e/test_phase4_other_sources.py::test_tiktok_ingestion` happy-path (native captions still work without sentinel)
