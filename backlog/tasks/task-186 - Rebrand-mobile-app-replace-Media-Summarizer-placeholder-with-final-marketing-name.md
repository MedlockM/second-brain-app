...
1. `mobile/app.config.ts:99` — `name: "Media Summarizer"` → display name de **fallback**, celui que voient les langues hors des 11 locales natives du point 5.
2. `mobile/app.config.ts:100` — `slug: "media-summarizer"` (kebab-case, doit matcher le dashboard Expo).
3. `mobile/app.config.ts:105` — `scheme: "media-summarizer"` (deep link).
4. `mobile/app.config.ts:241` — `iosShareExtensionName` : c'est le `CFBundleDisplayName` de la share extension, donc le **libellé de la ligne dans la iOS Share Sheet**. task-347 l'aligne sur le nom actuel de l'app ; ici il suit le nouveau nom. Le bundle id de l'extension dérive de celui de l'app (`<appId>.share-extension`) et ne bouge pas — ni App ID ni provisioning profile à recréer.
