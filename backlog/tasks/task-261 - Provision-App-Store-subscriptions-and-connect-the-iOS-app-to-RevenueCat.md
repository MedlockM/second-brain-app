...
5. **The webhook round trip is no longer a question — Android proved it on 2026-09-01.** A license-tester purchase on Play drove the full lifecycle (`INITIAL_PURCHASE`, five `RENEWAL`s, `CANCELLATION`, `EXPIRATION`) into `revenucat_events-dev`, with `subscriptions-dev` carrying `tier: L` resolved from the entitlement — see `task-238`, section « The full Android subscription lifecycle ran end to end ». That also settles what this gate used to list as unconfirmed: the shared secret in RevenueCat → Integrations → Webhooks **does** match `.env` and `media-summarizer-runtime-dev`, since nine signed requests got through a Lambda that answers `401 Invalid authorization` on a bad one. iOS uses the same endpoint and the same secret, so nothing here is iOS-specific any more. What still has to be checked on the iOS sandbox purchase is only what is platform-specific: that the event arrives with `store: app_store` and that `subscriptions-dev` records `platform: ios` with the right tier.
...
- **The three products read `subscription.duration: null`**, unlike the Test Store products which read `P1M`. That is the visible symptom of `app_store_connect_api_key_configured: false` — RevenueCat has never fetched the subscription. ~~It will flip to `P1M` once OWNER GATES 1 and 2 are done, and that is a cheap way to check them.~~ **Wrong prediction, measured 2026-09-02**: the subscriptions now exist in App Store Connect and the ASC API key is uploaded (`app_store_connect_api_key_configured: true`, vendor number filled), yet the three products still read `duration: null`. The check to use instead is the app-level flag. `duration` is a catalogue-import field, not a purchase-path one, and StoreKit resolving the products is what actually matters — proven the same day by real prices appearing on the TestFlight paywall.
- **In the dashboard the same cause shows as the status `Could not check`** on the three iOS rows. It reads literally: RevenueCat could not *perform* the verification, for want of a channel to query Apple — not "product invalid" nor "product not found". (The comparison with the Play rows offered here as proof no longer holds: as of 2026-09-01 the Play products carry `{"duration": "P1M", "grace_period_duration": "P7D"}`, not the `null` payload they had when this note was written. They were recreated once the Play service credentials were valid, which is precisely why their metadata got imported.) RevenueCat's own docs confirm what the missing key is for: « You may upload an App Store Connect API key for RevenueCat to import products and prices from App Store Connect. » Note that `subscription_key_configured` is `true`, so the credential that validates *purchases* is in place — this status is a catalog-verification signal, not a purchase blocker. Uploading the key alone will not turn it green either: OWNER GATE 1 has to happen too, otherwise the status just becomes "nothing found".
