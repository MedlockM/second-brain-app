...
Depends on `task-262`, which restructures the entitlement layout to one entitlement per tier and deletes the product-ID map. Do not start before it lands, or these products get wired to an entitlement that no longer exists.
...
- [ ] #3 Each iOS product is attached to its matching tier entitlement created by task-262 (tier_text_only, tier_mix, tier_audio_heavy), verifiable via GET /v2/projects/proj879a771a/entitlements/<id>/products
...
- [ ] #5 No store product identifier is added to media_summarizer/api/endpoints/revenucat_webhook.py: tier resolution stays entitlement-driven as delivered by task-262
