...
- **Ce contrôle porte sur le front-matter seul, jamais sur le corps de la tâche.** Un `grep dispatchable` sur le fichier entier ne prouve rien : beaucoup de descriptions évoquent le verrou en prose, le plus souvent celui d'une *autre* tâche (task-237 cite celui de task-252, task-254 celui de task-172), et task-282 dit même explicitement qu'elle n'est pas verrouillée. Lire ces mentions comme des verrous produit de faux écarts avec la denylist injectée. Limite la lecture aux lignes situées entre les deux `---` de tête.
- Ne signale donc un écart avec la denylist injectée que si le front-matter porte réellement la ligne. Le script valide déjà l'inverse — une tâche `To Do` dont le corps parle du verrou sans champ `dispatchable:` en front-matter interrompt le dispatch avant que tu sois lancé, parce que Backlog.md efface les champs personnalisés à chaque réécriture.
