...
| `test_transcript_translation.py` | task-192 : reading_language=fr + Reel Instagram anglais → artifact ET transcript brut (`/raw-content`) traduits en fr ; vérifie aussi que le pré-chauffage async (`media_completed_worker`) rend le premier `/raw-content` rapide (non-régression timeout API Gateway) |
