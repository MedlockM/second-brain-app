...
| 8 | **Uploaded document** | **The render of page 1 — corrected on 2026-09-03, see §11.1.** For a PDF, a DOCX or a PPTX: the `full_page_screenshot` the LlamaParse job we already poll produces, fetched from the finished job. For an XLSX: a first-sheet grid drawn from the parse output, because no provider on this path rasterises a spreadsheet (task-343 §4.2) | **Yes** — `ParseResult.metadata` already carries the `job_id` (`llamaparse_resolver.py:258`) and the markdown; no rasteriser was added to the Lambda image | `ParseResult.metadata` *may* carry a PDF `/Author`, provider-dependent and unverifiable here — treat as absent | No | Type icon: a document the Unstructured fallback parsed has no page render to fetch, and a corrupt or chart-only file has none to draw. Best-effort, never a failed ingestion |
...
**Totals**, as corrected on 2026-09-03. Cover already in hand and free: **5** (article, YouTube, podcast, TikTok, Instagram). One extra request parameter: **1** (X). Ours already, needs resizing: **2** (camera, gallery). Built from an artefact of the parse we already pay for, or drawn from it: **1** (document — task-343, task-344). Structurally impossible: **2** (shared text, audio file).
...
| **A PDF first-page thumbnail for documents** | ⚠️ **Superseded on 2026-09-03 and now implemented — see the note below.** This row said it needs a rasteriser in the Lambda image and produces a grey rectangle of text that identifies nothing. Both halves were wrong: task-343 found the page render is already produced by the LlamaParse job we pay for, and the owner overrode the second on visual evidence |
...
The row above rests on two claims. Both are now dead, and **§11 and §4 row 8 can no longer be cited to refuse a document cover** — it is implemented by `task-344`.

- **"Needs a rasteriser in the Lambda image"** — **false.** `task-343` verified against the live API that the LlamaParse job the worker already creates *and already pays for* renders every page of a PDF, a DOCX and a PPTX to a JPEG and exposes it on the job (§4.1 there), on the exact fields `_upload_file` sends today. The cover is two GETs on a finished job, not a new dependency: the owner's decision explicitly forbids adding one (option B of §8), so the Amazon Linux 2 / glibc 2.26 / arm64 constraint that let `task-304` reach `main` without ever running in a Lambda (`pyproject.toml:41-48`) is never touched. A spreadsheet is the exception — nothing on this path rasterises a grid — and its cover is drawn from the parsed table with the Pillow already in the `worker` extra.
- **"Produces a grey rectangle of text that identifies nothing"** — **false as a general claim.** In Apple's Files app, a passport, a carte Vitale, a driving licence and a branded cover page are each identifiable at thumbnail size. What §11 actually predicted is the outcome of *centre-cropping* a portrait page into the 16:9 tile of §6.4, which keeps a horizontal middle band and drops the header, logo and title. The owner's answer is to keep the 16:9 tile and align the crop to the **top** of the page — which `task-344` does server-side, in `cover_capture`, so the stored derivative is exactly 640x360 and every tile component inherits the framing (the one documented exception to "the crop is the client's job", §6.2 of `task-343`).

Scope set by the owner and delivered: **every** uploaded format (PDF, DOCX, PPTX, XLSX), not PDF alone. Carried by `task-343` (benchmark) and `task-344` (implementation); the decision to follow is the `Decision` field of `docs/research/task-343-document-page-render/README.md`. Existing `-dev` document rows keep their glyph until re-ingested (§11, "backfilling existing rows").
